...
def get_score(img, stype):
    global model
    print(model)
    results = model.predict(img.reshape(1, -1))
    sad = load_image(os.path.join(os.getcwd(), 'app', 'static', 'images', 'sad.jpg'))
    happy = load_image(os.path.join(os.getcwd(), 'app', 'static', 'images', 'happy.jpg'))
